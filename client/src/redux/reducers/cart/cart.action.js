...
export const getCart = () => async (dispatch) => {
  try {
    ...
    if (localStorage.delishGrabCart) {
      const { cart } = JSON.parse(localStorage.getItem("delishGrabCart"));
      cartData.cart = cart;
    }
...
export const addToCart = (newFood) => async (dispatch) => {
  try {
    ...
    localStorage.setItem("delishGrabCart", JSON.stringify({ cart: cartData.cart }));
